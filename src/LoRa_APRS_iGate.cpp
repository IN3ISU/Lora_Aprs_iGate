...
void setup()
{
	...
	{
		logPrintlnI("AXP192 Attivo!");
	}
	else
	{
		logPrintlnE("AXP192 Fallito!");
	}
	...
	delay(500);
	logPrintlnA("LoRa APRS iGate & Digi ");
	logPrintlnA("Version: 20.49.0-dev");
	setup_display();
	show_display("Ciao", "LoRa APRS iGate & Digi", "Eseguito", "20.49.0-dev", 3000);
...
void loop()
{
	...
	if(Config.wifi.active && WiFiMulti.run() != WL_CONNECTED)
	{
		...
		logPrintlnE("WiFi non connesso!");
		show_display("ERROR", "WiFi non connesso!");
		delay(1000);
		return;
	}
	...
	{
		setup_display(); secondsSinceDisplay = 0; display_is_on = true;
		logPrintI("connessione a APRS-IS server: ");
		logPrintI(Config.aprs_is.server);
		logPrintI(" on port: ");
		logPrintlnI(String(Config.aprs_is.port));
		show_display("INFO", "In Connessione to APRS-IS server");
		if(!aprs_is->connect(Config.aprs_is.server, Config.aprs_is.port))
		{
			logPrintlnE("Connessione Fallita.");
			logPrintlnI("Attende 5 Secondi prima di riprovare...");
			show_display("ERROR", "Connessione Sever Fallita!", "attendi 5 sec");
			delay(5000);
			return;
		}
		logPrintlnI("Connesso a APRS-IS server!");
	}
	...
	if(beacon_digi)
	{
		...
		logPrintlnD("Trasmissione finita...");
		show_display(Config.callsign, "Attendo...");
	}
	if(beacon_aprs_is)
	{
		beacon_aprs_is = false;
		setup_display(); secondsSinceDisplay = 0; display_is_on = true;
		show_display(Config.callsign, "Beacon a APRS-IS Server...");
		logPrintD("[" + timeClient.getFormattedTime() + "] ");
		logPrintlnD(BeaconMsg->encode());
...
void load_config()
{
	...
	logPrintlnI("Configurazione caricata!");
}
...
void setup_wifi()
{
	...
	{
		logPrintD("Attendo le AP: ");
		logPrintlnD(ap.SSID);
		WiFiMulti.addAP(ap.SSID.c_str(), ap.password.c_str());
	}
	logPrintlnI("Attendo per il WiFi");
	show_display("INFO", "Attendo per il WiFi");
	while(WiFiMulti.run() != WL_CONNECTED)
	{
		show_display("INFO", "Attendo per il WiFi", "....");
		delay(500);
	}
	logPrintlnI("WiFi connesso");
	logPrintD("IP address: ");
	logPrintlnD(WiFi.localIP().toString());
	show_display("INFO", "WiFi connesso", "IP: ", WiFi.localIP().toString(), 2000);
}
#endif
...
void setup_lora()
{
	...
	{
		logPrintlnE("Starting LoRa fallito!");
		show_display("ERROR", "Starting LoRa fallito!");
		while (1);
	}
